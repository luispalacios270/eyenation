  ...
  function toStandardCase(string) {
    ...
    return {
        ...
        controller: ["$scope", "$attrs", "$element", function($scope, $attrs, $element) {
            ...
            this.registerTagItem = function() {
                return {
                    ...
                    removeTag: function(index, t) {
                        if ($scope.disabled) {
                            return;
                        }

                        $scope.tagList.items[index] = t;
                        $scope.tagList.items.splice($scope.tagList.items.length - 1);
                    }
                };
            };
        }],
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            scope.$watch('tags', function(value, ol) {
                if (value) {
    ...
    return {
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            scope.$removeTag = function() {
              var t = scope.$parent.$parent.tags[scope.$parent.$parent.tags.length - 1];
              tagsInput.removeTag(scope.$index, t);
              scope.data = t;
            };

            scope.$on('langChange', function(event, value, ind){
              event.preventDefault();

              scope.data = event.targetScope.$parent.tag;
            });

            scope.$watch('$parent.tag.lang', function(value, ol) {
                if(value.toUpperCase() !== ol.toUpperCase() && ol !== undefined  ) {
             scope.$emit('langChange', value, scope.$parent.$index);
                }
            });
...
tagsInput.run(["$templateCache", function($templateCache) {
    ...
    "<div class=\"host\" tabindex=\"-1\" ng-click=\"eventHandlers.host.click()\" ti-transclude-append><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by $index\" ng-class=\"getTagClass(tag, $index)\" ng-click=\"eventHandlers.tag.click(tag)\"><ti-tag-item scope=\"$parent.$parent\" data=\"::tag\"></ti-tag-item></li></ul><input class=\"input\" autocomplete=\"off\" ng-model=\"newTag.text\" ng-model-options=\"{getterSetter: true}\" ng-keydown=\"eventHandlers.input.keydown($event)\" ng-focus=\"eventHandlers.input.focus($event)\" ng-blur=\"eventHandlers.input.blur($event)\" ng-paste=\"eventHandlers.input.paste($event)\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ng-disabled=\"disabled\" ti-bind-attrs=\"{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex, spellcheck: options.spellcheck}\" ti-autosize></div></div>"
  );
